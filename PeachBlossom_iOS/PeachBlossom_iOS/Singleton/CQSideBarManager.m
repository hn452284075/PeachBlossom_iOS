...
- (void)commonInit
{
    ...
    _endOffsetPoint = CGPointMake(_currentWindow.width, 0);
}
...
- (void)handleDeviceOrientationChange:(NSNotification *)notification
{
    ...
    _contentView.frame = CGRectMake(_startOffsetPoint.x, _startOffsetPoint.y, _currentWindow.width - _startOffsetPoint.x, _currentWindow.height - _startOffsetPoint.y);
}
...
- (void)openSideBar:(id<CQSideBarManagerDelegate>)delegate
{
    ...
    if (_delegate) {
        if ([_delegate respondsToSelector:@selector(viewForSideBar)]) {
             ...
             * 重置设置抽屉视图的初始位置
             */
            _contentView.left = _currentWindow.width;
        }
...
{
    [UIView animateWithDuration:SIDEBAR_ANIMATE_DURATION animations:^{
        _contentView.frame = CGRectMake(_endOffsetPoint.x, _endOffsetPoint.y,_currentWindow.width, _currentWindow.height);
