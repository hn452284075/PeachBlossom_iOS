...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.collectionGroupType == CollectionGroupType_SingleGroup) {
        ...
        if(self.firstSectionFootIdentifer){
            
            if (kind == UICollectionElementKindSectionFooter) {
                UICollectionReusableView *firstSectionFootView =  [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:self.firstSectionFootIdentifer forIndexPath:indexPath];
                
                return firstSectionFootView;
            }
...
//非xib
-(void)configureCollectionCellClassName:(Class)collectionCellClassName
             forCellWithReuseIdentifier:(NSString *)indentifier
            configureCollectionCellData:(ConfigureCollectionCellBlock)configureCollectionCellBlock
                              clickCell:(ClickCellBlock)clickCellBlock
{
    
     [self configureCollectionCellClassName:[collectionCellClassName class] forCellWithReuseIdentifier:indentifier cellDataSource:nil configureCollectionCellData:configureCollectionCellBlock clickCell:clickCellBlock];
...
-(void)configureCollectionCellClassName:(Class)collectionCellClassName
            ...
            configureCollectionCellData:(ConfigureCollectionCellBlock)configureCollectionCellBlock
                              clickCell:(ClickCellBlock)clickCellBlock{
    self.cellIdentifer = [indentifier stringByAppendingString:@"ID"];
    [self registerClass:[collectionCellClassName class]  forCellWithReuseIdentifier:self.cellIdentifer];
    self.dataLogicModule.currentDataModelArr = [cellDataSource mutableCopy];
    self.configureCollectionCellBlock = configureCollectionCellBlock;
    self.clickCellBlock = clickCellBlock;
...
//非xib
- (void)configureFirstSectioHeaderClassName:(Class)firstSectionHeaderClassName
                 forCellWithReuseIdentifier:(NSString *)indentifier
           configureFirstSectionHeaderBlock:(ConfigurefirstSectionHeaderBlock)configurefirstSectionHeaderBlock{
    self.configurefirstSectionHeaderBlock = configurefirstSectionHeaderBlock;
    self.firstSectionHeaderIdentifer = [indentifier stringByAppendingString:@"ID"];
    
    [self registerClass:[firstSectionHeaderClassName class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:self.firstSectionHeaderIdentifer];
    
}

#pragma mark - 配置firstSectionFootView---不需要点击暂时不加回调
- (void)configureFirstSectioFootClassName:(Class)firstSectionFootClassName
                 forCellWithReuseIdentifier:(NSString *)indentifier{
    
    self.firstSectionFootIdentifer = [indentifier stringByAppendingString:@"ID"];
    
    [self registerClass:[firstSectionFootClassName class] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:self.firstSectionFootIdentifer];
